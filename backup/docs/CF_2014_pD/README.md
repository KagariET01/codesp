# [`CF 2014 pD`](https://codeforces.com/contest/2014/problem/D) Robert Hood and Mrs Hood
## 標籤
...
int main(){
	...
	while(t--){
		...
		INT mnp=1,mxp=1,nw=0;
		INT mn=1e9+7,mx=0;
		for(INT i=1;i<=n;i++){
			nw+=lst[i];
			if(i>=d){
				if(nw<mn)mnp=i-d+1,mn=nw;
				if(mx<nw)mxp=i-d+1,mx=nw;
				nw-=lst2[i-d+1];
			}
		}
		cout<<mxp<<" "<<mnp<<endl;
