# [`CF 2036 pC`](https://codeforces.com/contest/2036/problem/C) Anya and 1100
## 標籤
...
bool checker(string&str,INT p){
	for(INT i=0;i<4;i++){
		if(cs[i]!=str[p+i])return false;
	}
	return true;
}
...
int main(){
	...
	while(t--){
		...
		while(q--){
			...
			p--;
			str[p]=v;
			for(INT i=max(0ll,p-3);i<=p && i+3<n;i++){
				ans-=c[i];
				c[i]=checker(str,i);
				ans+=c[i];
			}
