# [`AT abc351 pD`](https://atcoder.jp/contests/abc351/tasks/abc351_D) Grid and Magnet
## 標籤
...
int main(){
	...
	for(INT i=0;i<n;i++){
		for(INT j=0;j<m;j++){
			if(mv[i][j])continue;
			if(mp[i][j]==1){maxs(ans,1ll);continue;}
			if(mp[i][j]==2)continue;
			vector<PII>vec;//record mp[x][y]==1
			deque<PII>dq;
			INT cnt=0;
