# [`CF 2033 pE`](https://codeforces.com/contest/2033/problem/E) Sakurako, Kosuke, and the Permutation
## 標籤
	...
	while(t--){
		...
		for(INT i=0;i<n;i++){
			if(a[a[i]]==i)continue;
				...
				INT nwnum=a[i];
				INT nwnum_idx=idx[nwnum];
				INT swap_num=a[a[i]];
				INT swap_num_idx=idx[swap_num];
				INT target_num=i;
				INT target_num_idx=idx[target_num];

				swap(a[swap_num_idx],a[target_num_idx]);
				idx[a[swap_num_idx]]=swap_num_idx;
				idx[a[target_num_idx]]=target_num_idx;
				ans++;
			}
		}
		cout<<ans<<endl;
