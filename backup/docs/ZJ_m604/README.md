# [`ZJ m604`](https://zerojudge.tw/ShowProblem?problemid=m604) 112北二4a.訂購臘腸 (Sausage)
## 標籤
...
int main(){
	...
	sort(id,id+m,[](vector<INT>&a,vector<INT>&b){return a.size()>b.size();});
	...
	for(INT i=0;i<m;i++){
		if(i&1){
			...
			for(INT j=id[i].size()-1;j>=0;j--){
				take.push_back(id[i][j]);
			}
		}
	}
	//cerr<<take<<endl;
	vector<vector<INT>>vec(can[ktt]+1,vector<INT>(0,0));

	INT nw=can[ktt];
	INT it=0;
	while(nw){
		while(!vec[nw].empty())nw--;
		INT mx=0;
		INT nwit=it;
		while(nwit-it<nw){
			if(take[nwit]==-1)break;
			nwit++;
		}
		mx=nwit-it;
		//cerr<<pit(nwit)<<pit(it)<<pit(mx)<<pit(nw)<<endl;
		while(it<nwit){
			vec[mx].push_back(take[it]);
			it++;
		}
		if(take[it]==-1)it++;
		
	}
	for(INT i=1;i<=can[ktt];i++){
		cout<<vec[i]<<endl;
	}
	//cout<<vec<<endl;
	return 0;
