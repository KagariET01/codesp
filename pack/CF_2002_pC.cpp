...
FINT getlen(PFIFI a,PFIFI b){
	return (a.F-b.F)*(a.F-b.F)+
		(a.S-b.S)*(a.S-b.S);
}

PFIFI len(PFIFI nw,PFIFI l,PFIFI r){
	INT t=1000;
	while(t--){
		PFIFI mid1=(r-l)/3.0+l;
		PFIFI mid2=(r-l)*2.0/3.0+l;
		FINT len1=getlen(nw,mid1);
		FINT len2=getlen(nw,mid2);
		if(len1<len2)r=mid2;
		else if(len1>len2)l=mid1;
		else r=mid2,l=mid1;
	}
	return l;
}
...
int main(){
	...
	while(t--){
		...
		for(INT i=0;i<n;i++){
			// 確認點線最短距
			if(0){
				PFIFI gl=len(a[i],s,t);
				FINT tme=getlen(gl,s);
				FINT cr=getlen(gl,a[i]);
				if(tme<cr)ans&=1;
				else{
					ans&=0;
					break;
