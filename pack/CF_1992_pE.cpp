...
int main(){
	...
	while(t--){
		...
		for(INT len=1;len<=10000;len++){
			...
			if(nwaa==0 || nwa%nwaa){
				if(gt==len)
				for(nwa=1;nwa<=10000;nwa++){
					INT nwb=nwa*nlen-len;
					if(1<=nwb && nwb<=min(10000ll,nwa*ni)){
						ans.push_back(PII(nwa,nwb));
						//cerr<<pit(nwa)<<pit(nwb)<<endl;
					}
				}
				continue;
			}
