...
int main(){
	...
	cin>>vec;
	INT mid=10000;
	vector<PII>vec2[20000];
	for(auto&i:vec2){
		i.push_back(PII(-1,-1));
	}
	vec2[mid].push_back(PII(0,0));
	INT ans=0;
	INT nw=0;
	INT odec=mid;
	//cerr<<"check r"<<endl;
	for(INT i=n-1;i>=0;i--){
		...
		nw+=vec[i];
		odec+=oddeven(vec[i]);
		//cerr<<pit(nw)<<pit(odec);
		if(nw>k)break;
		if(odec==mid && nw<=k){
			maxs(ans,nw);
		}
		vec2[odec].push_back(PII(n-i,nw));
		////cerr<<vec2[odec].back()<<endl;
	}
	for(auto&i:vec2){
		sort(i);
	}
	...
	for(INT i=0;i<n;i++){
		...
		vector<PII>&fv=vec2[mid-odec];
		vector<PII>::iterator it=upper_bound(fv.begin(),fv.end(),PII(n-(i+1),k-nw));
		// it--;
		vector<PII>::iterator l,r;
		r=it;
		l=fv.begin();
		while(l<r){ 
			vector<PII>::iterator mid=(r-l)/2+l+1;
			if(mid->S+nw>k){
				r=mid-1;
			}else{
				l=mid;
