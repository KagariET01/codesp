...
PIB get_cnt(INT nw,INT lst,INT deep_need){
	INT re=0;
	bool meadd=1;
	if(deep[nw]==deep_need)meadd=0;
	for(auto&i:tree[nw]){
		if(i==lst)continue;
		PIB nwv=get_cnt(i,nw,deep_need);
		re+=nwv.F;
		meadd&=nwv.S;
	}
	if(meadd || deep[nw]>deep_need)re++;
	return PIB(re,meadd);
}
...
int main(){
	...
	while(t--){
		ans=1e9+7;
		cin>>n;
		for(INT i=0;i<n+3;i++)tree[i].clear();
		for(INT i=1;i<n;i++){
			INT x,y;
			cin>>x>>y;
			tree[x].push_back(y);
		...
		while(r-l<5){
			INT mid1=(r-l)/3+l;
			INT mid2=(r-l)/3*2+l;
			INT g1=get_cnt(1,0,mid1).F;
			INT g2=get_cnt(1,0,mid2).F;
			mins(ans,min(g1,g2));
			if(g1<g2)r=mid2;
			else l=mid1;
		}
		for(INT i=l;i<r;i++){
			INT nw=get_cnt(1,0,i).F;
			mins(ans,nw);
		}
		cout<<ans<<endl;
	}
