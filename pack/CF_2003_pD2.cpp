...
int main(){
	...
	while(t--){
		...
		vector<INT>
			link(n,-1), // 該點最遠能連到哪裡
			bascnt(n,-1), // 有多少個序列的mex為i
			ultcnt(n,-1); // 有多少序列可以將i轉移到link[i]
		// 當basid[i] || ultid[i] == -2時，代表有>=2個序列可以do this thing
		for(INT i=0;i<n;i++){
			INT sz=read(INT);
			bool a[mxa]={};
			for(INT j=0;j<sz;j++){
			...
			for(INT j=0;j<mxa;j++){
				if(!a[j]){
					if(!status){
						...
						if(j>link[st])ultcnt[st]=0,link[st]=j;
						if(j>=link[st])ultcnt[st]++
					}
				}
			}
		}
