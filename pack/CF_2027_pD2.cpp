...
int main(){
	...
	while(t--){
		...
		for(INT i=0;i<=n;i++){
			for(INT j=1;j<=m;j++){
				if(j==1)tp[i][j]=n;
				else tp[i][j]=tp[i][j-1];
				while(atot[tp[i][j]]-atot[i]>b[j])tp[i][j]--;
			}
		}
		PII dp[n+1][m+1]={};
		for(INT i=0;i<=n;i++){
		...
		for(INT i=0;i<=n;i++){
			for(INT j=1;j<=m;j++){
				if(dp[i][j-1].F<dp[i][j].F){
					dp[i][j]=dp[i][j-1];
				}else if(dp[i][j-1].F==dp[i][j].F){
					dp[i][j].S+=dp[i][j-1].S;
				}
				if(dp[i][j].S==-1)continue;
				if(tp[i][j]!=i){
					if(dp[tp[i][j]][j].F>dp[i][j].F+m-j){
						dp[tp[i][j]][j]=dp[i][j];
						dp[tp[i][j]][j].F+=m-j;
					}else if(dp[tp[i][j]][j].F==dp[i][j].F+m-j){
						dp[tp[i][j]][j].S+=dp[i][j].S;
						dp[tp[i][j]][j].S%=MOD;
					}
				}
				dp[i][j].S%=MOD;
		...
		for(INT i=0;i<=n;i++){
			for(INT j=0;j<=m;j++){
				cerr<<(dp[i][j].F>1e9?PII(-1,-1):dp[i][j])<<"\t";
			}
			cerr<<endl;
		}
		cerr<<endl;
		#endif
		if(dp[n][m].S<=-1)cout<<-1<<endl;
		else cout<<dp[n][m].F<<" "<<dp[n][m].S<<endl;
	}
	return 0;
}
