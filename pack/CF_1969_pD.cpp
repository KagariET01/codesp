...
int main(){
	...
	while(t--){
		...
		for(INT i=0;i<n;i++){
			if(vec[i].first>=vec[i].second)vec[i]=PII(0,0);
		}
		sort(vec.begin(),vec.end(),[](PII&a,PII&b){if(a.second!=b.second)return(a.second<b.second);else{return(a.first<b.first);}});
		INT ans=0;
		INT nw=0;
		for(PII&i:vec){//buy all first
			nw-=i.first;
			nw+=i.second;
		}
		for(INT i=0;i<k;i++){
		...
		INT ik=k;
		while(1){
			if(it<0 || it-ik<0 || ik<=0)break;
			if(vec[it-ik]==PII(-1,-1))break;
			INT delta=0;
			delta+=vec[it].first;//now I dont want to buy i
			delta-=vec[it-ik].second;//Bob can get one more for free
			if(delta>=0)nw+=delta;
			else ik--;
			maxs(ans,nw);
			it--;
