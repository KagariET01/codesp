...
str edge[mxn*mxn];
bool edge_vis[mxn*mxn];
bool p_vis[2][mxn];
INT len[2][mxn];

istream&operator>>(istream&in,str&s){
	return in>>s.x>>s.y>>s.v;
}
...
void dfs(INT nw,INT lv=0){
	...
	if(nw==1&&lv==1){
		if(ans2==-1)ans2=len[lv][nw];
		else mins(ans2,len[lv][nw]);
	}
	for(INT&i:tree[nw]){
		if(edge_vis[i])continue;
		INT nx=edge[i].x;
		if(nx==nw)nx=edge[i].y;
		if(p_vis[lv][nx])continue;
		if(1){
			len[lv][nx]=len[lv][nw]+edge[i].v;
			edge_vis[i]=1;
			dfs(nx,lv);
			edge_vis[i]=0;
		}
...
int main(){
	...
	for(INT i=0;i<=n;i++)len[0][i]=len[1][i]=1e9+7;
	len[0][1]=0;
	dfs(1,0);
