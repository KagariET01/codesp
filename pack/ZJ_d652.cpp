...
int main(){
	...
	for(INT i=0;i<n;i++){
		for(INT j=0;j<n;j++){
			dp[i][j]=1e9+7;
			if(abs(i-j)<=2)dp[i][j]=0;
		}
	}
	for(INT i=0;i<n-2;i++){
		dp[i][i+2]=vec[i]*vec[i+1]*vec[i+2];
	}

	for(INT l=0;l<n;l++){
		for(INT r=l+1;r<n;r++){
			for(INT k=l+1;k<r-1;k++){
				mins(dp[l][r],dp[l][k]+dp[k+1][r]+min(
							vec[l]*vec[k]*vec[k+1] + vec[l]*vec[k+1]*vec[r],
							vec[k]*vec[k+1]*vec[r] + vec[l]*vec[k]*vec[r]
							));
			}
		}
	}
