# [`CF 2028 pD`](https://codeforces.com/contest/2028/problem/D) Alice's Adventures in Cards
## 標籤
...
int main(){
	...
	while(t--){
		...
		cin>>a;
		vector<PCI>sol(n,PCI('\0',-1));
		vector<INT>minid(3,n-1); // 可拿來交易的、且比i還要大、且對j而言價值最小的牌
		// 目標是交易到編號n的牌
		...
		for(INT i=n-2;i>=0;i--){
			INT win=-1;
			for(INT j=0;j<3;j++){
				if(a[j][i]>a[j][minid[j]])win=j; // 表編號為i的牌可以和第j的人換取到mins[j]的牌
				// 現在，只要拿到牌i，我們可以直接換到mins[j]的牌
			}
			if(win==-1)continue;
			// 確定可以用牌i換到後面更大的牌，可行目標多一：得到牌i
			sol[i]=PII(str[win],minid[win]);
			for(INT j=0;j<3;j++){
				if(a[j][i]<a[j][minid[j]])minid[j]=i;
			}
		}
		if(sol[0].S==-1){ // 如果牌1換不到目標牌： output NO
			cout<<"NO"<<endl;
			continue;
		}
		cout<<"Yes"<<endl;
		...
		ans.push_back(sol[0]);
		while(1){
			if(ans.back().S==n-1)break;
			ans.push_back(sol[ans.back().S]);
		}
		cout<<ans.size()<<endl;
		for(auto&i:ans){
			cout<<i.F<<" "<<i.S+1<<endl;
