# [`CF 2020 pD`](https://codeforces.com/contest/2020/problem/D) Connect the Dots
## 標籤
...
namespace ET01{
	struct Dsu{
		...
		void Init(INT sz){
			vec.resize(sz);
			for(INT i=0;i<sz;i++)vec[i]=i;
		}
		INT Find(INT n){
			if(vec[n]==n)return n;
			else return vec[n]=Find(vec[n]);
		}
		...
		for(INT i=1;i<=10;i++){
			for(INT j=1;j<=n;j++){
				if(lst[i][j]){
					if(i+j<=n){
						dsu.Union(j,i+j);
						maxs(lst[i][i+j],lst[i][j]-1);
					}
				}
			}
		}
		set<INT>se;
		for(INT i=1;i<=n;i++)se.insert(dsu.Find(i));
		cout<<se.size()<<endl;
		#ifdef DBG
			if(0){
				for(INT i=0;i<=10;i++){
