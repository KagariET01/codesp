# [`CF 1987 pD`](https://codeforces.com/contest/1987/problem/D) World is Mine
## 標籤
...
int main(){
	...
	while(t--){
		...
		for(INT i=1;i<m;i++){//Alice eat
			for(INT j=0;j<m;j++){//Bob can eat
				if(j+b[i]<i+1) maxs(dp[i][j],dp[i-1][j+b[i]]+1);
				if(0<=j-1)   maxs(dp[i][j],dp[i-1][j-1]);
				maxs(dp[i][j],dp[i-1][j]);
			}
		}
		//cerr<<pit(dp)<<endl;
		cout<<m-dp[m-1][0]<<endl;
	}
	return 0;
}
