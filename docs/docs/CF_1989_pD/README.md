# [`CF 1989 pD`](https://codeforces.com/contest/1989/problem/D) Smithing Skill
## 標籤
...
const INT mxn=2e6+2;
const INT mxn2=1e6+5;
INT lst[mxn],lst2[mxn];
...
int main(){
	...
	for(INT i=0;i<n;i++){
		...
		mins(lst[a[i]],a[i]-b[i]);
		mins(mnmn,a[i]-b[i]);
	}
	for(INT i=1;i<mxn;i++){
		if(lst[i]==-1)lst[i]=lst[i-1];
		if(!(lst[i-1]==-1))mins(lst[i],lst[i-1]);
	}
	for(INT i=1;i<mxn;i++){
		if(lst[i]!=-1)lst2[i]=lst2[i-lst[i]]+1;
	}
	vector<INT>ans(m);
	INT anstt=0;
	for(INT i=0;i<m;i++){
