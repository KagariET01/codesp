# [`CF 2000 pE`](https://codeforces.com/contest/2000/problem/E) Photoshoot for Gorillas
## 標籤
	...
	while(t--){
		...
		for(INT i=1;i<=n;i++){
			for(INT j=1;j<=m;j++){
				INT spl=min(i-1,k-1);
				INT spr=min(n-i,k-1);
				INT wid=spl+spr+1-(k-1);
				mins(wid,k);
				maxs(wid,1ll);
				INT spu=min(j-1,k-1);
				INT spd=min(m-j,k-1);
				INT hei=spu+spd+1-(k-1);
				mins(hei,k);
				maxs(hei,1ll);
				INT nw=wid*hei;
				#ifdef DBG
					cerr<<pit(i)<<pit(j)<<pit(spl)<<pit(spr)<<pit(spu)<<pit(spd)<<pit(wid)<<pit(hei)<<endl;
				#endif
				pv.push_back(make_pair(nw,&(mp[i][j])));
			}
		}
		sort(pv);
		...
		for(INT i=1;i<=n;i++){
			for(INT j=1;j<=m;j++){
				totmp[i][j]=totmp[i-1][j]+totmp[i][j-1]-totmp[i-1][j-1]+mp[i][j];
			}
		}
		INT ans=0;
		for(INT i=k;i<=n;i++){
			for(INT j=k;j<=m;j++){
				INT nw=totmp[i][j]-totmp[i-k][j]-totmp[i][j-k]+totmp[i-k][j-k];
				ans+=nw;
			}
		}
