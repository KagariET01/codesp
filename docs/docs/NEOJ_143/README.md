# [`NEOJ 143`](https://neoj.sprout.tw/problem/143/) 合成円円
## 標籤
...
int main(){
	...
	while(t--){
		...
		INT tot=0;
		vector<INT>tt;
		for(INT&i:vec)tt.push_back(tot+=i);
		INT dp[n+1][n+1]={};
		for(INT i=0;i<=n;i++){
			for(INT j=0;j<=n;j++){
				dp[i][j]=1e9+7;
		...
		for(INT i=0;i<n;i++){//len
			for(INT j=1;j<=n-i;j++){//st
				for(INT k=0;k<i;k++){
					mins(dp[j][j+i],dp[j][j+k]+dp[j+k+1][j+i]);
				}
				dp[j][j+i]+=tt[j+i]-tt[j-1];
			}
